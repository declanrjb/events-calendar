...
links_list <- page %>% html_nodes('#mainContent') %>% html_nodes("p") %>% html_children() %>% html_attr("href")
...
# latest blotter happens to be misconfigured on Reed's end, second latest provides a working example
url <- links_df[2,]
...
write.csv(df,"data/blotter.csv",row.names=FALSE)
write.csv(lead_df,"data/lead.csv",row.names=FALSE)
